...
- Location: tests\Login.spec.js:12:7
...
Locator: locator('[data-test="title"]').first()
...
```ts
  ...
  2  | 
  3  | import loginData from '../testdata/loginData.json' assert { type: 'json' };
  4  | 
  5  | import LoginPage from '../pages/LoginPage.js';
  6  | 
  7  | import { attachStepScreenshot } from '../utils/screenshotUtil.js';
  8  | 
  9  | 
  10 | test.describe('Login', () => {
  11 | 
  12 |   test('Login Test Case with valid user', async ({ page }) => {
  13 | 
  14 |     const loginPage = new LoginPage(page);
  15 | 
  16 |     const data = loginData.loginTests[0];
  17 | 
  18 |     await test.step('Step 1 - Enter credentials and login', async () => {
  19 | 
  ...
  30 |     });
  31 | 
  32 |     await test.step('Step 2 - Verify Welcome Message on Landing page', async () => {
  33 | 
  34 |       await expect(loginPage.message.first())
> 35 |         .toHaveText(data.ExpectedMsg);
     |          ^ Error: expect(locator).toHaveText(expected) failed
  36 | 
  37 |       await attachStepScreenshot(
  38 |         page,
  39 |         '02 - After welcome message verification'
  40 |       );
  41 | 
  42 |     });
  43 | 
